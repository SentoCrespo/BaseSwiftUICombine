...
public extension UIView {
    func applyArrowDialogAppearanceWithOrientation(arrowOrientation: UIImage.Orientation, arrowLength: CGFloat = 20.0) {
        let shapeLayer = CAShapeLayer()
        shapeLayer.path = dialogBezierPathWithFrame(
            self.frame,
            arrowOrientation: arrowOrientation,
            arrowLength: arrowLength).cgPath
        shapeLayer.fillColor = self.backgroundColor?.cgColor ?? UIColor.white.cgColor
